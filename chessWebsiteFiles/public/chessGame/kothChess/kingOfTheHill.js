...
const socket = io('/kothGame');

let opponentUsername;
let opponentFound = false;
let currentRoomIndex = sessionStorage.getItem("kothCurrentRoom");
let currentPosition = sessionStorage.getItem("kothCurrentPosition");
...
if(currentRoomIndex !== null && currentPosition !== null && username !== null){
	...
	currentRoomIndex = parseInt(sessionStorage.getItem("kothCurrentRoom"));
	currentPosition = parseInt(sessionStorage.getItem("kothCurrentPosition"));
...
socket.on('clientDisconnect', (data) => {
	...
	sessionStorage.removeItem("kothCurrentRoom");
	sessionStorage.removeItem("kothCurrentPosition");
	disconnected = true;
...
socket.on('clientHillWin', (data) => {
	...
	if(data.hillWin){
		...
		//data.player is losing player
		if(data.player === 1){
			printText("Black King On The Hill, Black Wins");
		}

		else{
			printText("White King On The Hill, White Wins");
		}
...
function move(e){
	...
	if(selectedPiece === "empty"){
		...
		if(backendBoard.kingOfTheHill(currentPlayer)){
			let hillWinMove = {
			...
			socket.emit('serverHillWin', hillWinMove);
			messageSent = true;
		}


		else if(checkStatus.inCheck && checkStatus.side === opponentPlayer){
...
function createBoard(){
	...
	for(let i = 0; i < width; i++){
		...
		for(let j = 0; j < height; j++){
			...
			if(i === 3 || i === 4){
				if(checkCenterOfBoard(i, j)){
					spot.className = "w-1/8 h-1/8 border border-black border-solid bg-yellow-300 m-0";
				}
			}
...
function checkCenterOfBoard(x, y){
