...
const socket = io('/speedGame');

let opponentUsername;
let opponentFound = false;
let currentRoomIndex = sessionStorage.getItem("speedCurrentRoom");
let currentPosition = sessionStorage.getItem("speedCurrentPosition");
...
if(currentRoomIndex !== null && currentPosition !== null && username !== null){
	...
	currentRoomIndex = parseInt(sessionStorage.getItem("speedCurrentRoom"));
	currentPosition = parseInt(sessionStorage.getItem("speedCurrentPosition"));
...
socket.on('getOppUsername', (data) => {
	...
	if(data.playerUsername === username){
		return;
	}

	if(data.oppFound === true){
		opponentFound = true;
		oppUsernameText.innerHTML = data.oppUsername;
		opponentUsername = data.oppUsername;
		clearText();
...
socket.on('opponentLeft', (data) => {
	...
	//drawBtn.setAttribute("value", "Offer Draw");
	//drawBtn.innerHTML = "Offer Draw";
...
socket.on('clientDisconnect', (data) => {
	...
	console.log("Inside Client Disconnect");

	sessionStorage.removeItem("speedCurrentRoom");
	sessionStorage.removeItem("speedCurrentPosition");
	disconnected = true;
...
socket.on('timeLoss', (data) => {
	...
	console.log("Got to TimeLoss");
	console.log(data);
	console.log("Registering a Time Loss.");
...
socket.on('clientCheckInsufficientMaterial',  (data) => {
	...
	console.log("Made it here to client insufficient material");
	...
	if(data.player === whichPlayer){
		...
		*/
		...
		console.log("Draw set status: " + data.drawSet);
		console.log("About to be sent to confirm Insufficient Check");
		socket.emit('confirmInsufficientCheck', data);
...
socket.on('clientDraw', (data) => {
	...
	if(data.player === whichPlayer && data.handShakeComplete === false){
		//just an offer that I already sent, no need to change anything
		return;
	}
	...
	if(data.handShakeComplete === false){
		...
		//drawBtn.setAttribute("value", "Accept Draw");
...
socket.on('clientReset', (data) => {
	...
	if(data.handShakeComplete === false){
		...
		//rematchBtn.setAttribute("value", "Accept Rematch");
...
socket.on('clientMessage', (data) => {
	...
	if(data.player === whichPlayer){
		...
		console.log("Opponent Time Left: ");
		console.log(data.oppTimeLeft);
...
function move(e){
	...
	if(selectedPiece === "empty"){
		...
		if(updateStat.getMoveType() === "promotion"){
			savedMove.push(selectedX);
			savedMove.push(selectedY);
			savedMove.push(xVal);
			savedMove.push(yVal);
			openShowcase();
			console.log("Opened Showcase");
			return;
		}
		...
		//sendingMove.player = whichPlayer;
...
function finishMove(messageSent){
...
function resetBasics(){
	...
	//sendingMove.player = -1;
	sendingMove.sendingMoveSet.splice(0, sendingMove.sendingMoveSet.length);
	sendingMove.moveType = null;
	sendingMove.promoteDOM = null;
	sendingMove.promotePiece = null;

	savedMove.splice(0, savedMove.length);

	//rematchBtn.setAttribute("value", "Rematch");
	...
	myTimer.innerHTML = "01:00";
	oppTimer.innerHTML = "01:00";
...
//frontend board manipulation functions
...
function updateDOMBoardAndSaveMove(moveType, moveSet){
	...
	if(moveType === "regular"){
		...
		let domPiece1 = removeDOMPiece(moveSet[0], moveSet[1]);
		insertDOMPiece(domPiece1, moveSet[2], moveSet[3]);
		let domPiece2 = removeDOMPiece(moveSet[0], moveSet[4]);
		insertDOMPiece(domPiece2, moveSet[0], moveSet[5]);
		gameSaver.addCastleMove(moveSet[0], moveSet[1], moveSet[2], moveSet[3], moveSet[4], moveSet[5]);
		gameSaver.incrementCount();
		updateSaveGameModalSpans();
	}
...
function turnTimerRed(whichTimer){
	...
	//if(whichTimer === "myTimer" && myTimerRed === false && myMinutes <= 0 && mySeconds < 10){
	//if(whichTimer === "myTimer" && myTimerRed === false && ((myMinutes < 0) || (myMinutes === 0 && mySeconds < 10)) ){
	...
	//else if(whichTimer === "oppTimer" && oppTimerRed === false && oppMinutes <= 0 && oppSeconds < 10){
	//else if(whichTimer === "oppTimer" && oppTimerRed === false && ((oppMinutes < 0) || (oppMinutes === 0 && oppSeconds < 10))){
...
function handleCountDownClock(whichTimer){
	...
	if(whichTimer === "myTimer"){
		myStartTime = (myMinutes * 60) + mySeconds;
		countDownMyClock();
	}
...
function countDownOppClock(){
	...
	if(myCountDown !== null){
		...
		myCountDown = null;
		secondsPassed = myStartTime - ((myMinutes * 60) + mySeconds);
		if(secondsPassed <= 2 && myMinutes === 0 && mySeconds <= 29 && myExtraSeconds > 0 ){
			mySeconds = mySeconds + 3;
			myExtraSeconds = myExtraSeconds - 3;
			//myTimer.innerHTML = getTimerFormat(myMinutes, mySeconds);
		}
		//mySeconds = mySeconds + 5;
		myTimer.innerHTML = getTimerFormat(myMinutes, mySeconds);
		turnTimerRed("myTimer");
	}
...
function rookPromote(e){
	...
	if(whichPlayer === 1){
		promoteDOMPiece = "whiteRook";
		promotePiece = "wR";
	}
	else{
		promoteDOMPiece = "blackRook";
		promotePiece = "bR";
	}
}
...
function bishopPromote(e){
	...
	if(whichPlayer === 1){
		promoteDOMPiece = "whiteBishop";
		promotePiece = "wB";
	}
	else{
		promoteDOMPiece = "blackBishop";
		promotePiece = "bB";
	}
}
...
promotionSelectBtn.onclick = function(){
	...
	backendBoard.pawnPromoteMove(promotePiece, savedMove[0], savedMove[1], savedMove[2], savedMove[3]);

	removeDOMPiece(savedMove[0], savedMove[1]);

	insertDOMPiece(promoteDOMPiece, savedMove[2], savedMove[3]);

	gameSaver.addPromotionMove(promotePiece.charAt(1), savedMove[0], savedMove[1], savedMove[2], savedMove[3]);
	gameSaver.incrementCount();
	updateSaveGameModalSpans();

	for(let i = 0; i < savedMove.length; i++){
		sendingMove.sendingMoveSet.push(invertMoves(savedMove[i]));
	}

	savedMove.splice(0, savedMove.length);

	//sendingMove.player = whichPlayer;
	sendingMove.moveType = "promotion";
	...
	messageSent = sendAdditionalDrawCondition(noProgression, messageSent, "promotion");

	if(promotionModal.classList.contains('hidden') === false){
		promotionModal.classList.add('hidden');
	}

	finishMove(messageSent);
}
...
function openSaveGameModal(){
	...
	/*
	if(gameInProgress){
		badSaveWarning.innerHTML = "Cannot Save A Game In Progress";
	}
	
	if(firstGameComplete === false && disconnected){
		badSaveWarning.innerHTML = "Cannot Save: No Game Has Been Completed And You Are Disconnected";
	}
	*/
...
finishSaveBtn.onclick = function(){
	...
	if(gameInProgress){
		badSaveWarning.innerHTML = "Cannot Save A Game In Progress";
		printText("Game In Progress: Game Not Saved");
		return;
	}

	if(firstGameComplete === false && disconnected){
		badSaveWarning.innerHTML = "Cannot Save: No Game Has Been Completed And You Are Disconnected";
		printText("No Complete Game: Game Not Saved");
		return;
	}

	if(saveGameNameInput.value.length === 0){
		badSaveWarning.innerHTML = "Must Enter Name";
		return;
	}
	*/
	...
	console.log("Completely Finished Save Game Process");
...
async function sendGameData(nameOfGame){
	...
	if(fetchResult === 0){
		...
		printText("Game Not Saved: You've Reached The Limit Of Saved Games.")
	}
	*/
